<html>
	<head>
		...
		if($_GET['acao']=="cad" || isset($_GET['id'])){
			...
			if(isset($_GET['id'])){
				...
				$nomeEmpresaFechado = $result['NomeEmpresa'];
	...
	<body>
		...
		Prezado(a): Beltrano da Silva<br>
		...
		if(isset($_GET['id'])){
			...
			$linha = '<tr class="comum">'
					.'<td align="center">'.$posicao_item.'<img src=\'img/edit.png\' class=\'editEmpresa\' onclick=\'editarItem("'.$posicao__item.'","'.$_GET['id'].'")\'></td>'
					.'<td>'.nl2br($descricao_item).'</td>'
					.'<td>'.$Ncm_item.'</td>'
					.'<td>'.$vlrUnit_item.'</td>'
					.'<td>'.$icms_item.'</td>'
					.'<td>'.$ipi_item.'</td>'
